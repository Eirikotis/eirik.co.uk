# Technical background

Eirik is technically hands-on, but his depth varies by domain. Describe specific evidence rather than assigning a broad software-engineer label.

## Strong direct evidence

- Next.js and React web products;
- TypeScript and Node.js services;
- Python data and quantitative-research workflows;
- SQL and relational data work;
- REST and protocol API integration;
- on-chain Solana data and transaction workflows;
- market-data ingestion, cleaning, normalisation and validation;
- quantitative modelling, backtesting and portfolio simulation;
- Ridge and XGBoost experiments;
- walk-forward validation, frozen holdouts and leakage controls;
- systemd, Ubuntu/VPS deployment and ongoing operations;
- automation, state machines, idempotency and crash recovery;
- LLM-assisted implementation and bounded research workflows;
- product requirements and engineering coordination.

## Systems judgement

Eirik often works by connecting the product decision to the underlying system: data sources, incentives, execution paths, operating dependencies and failure states. Evidence includes transaction validation and recovery in dusd.fun, model and market-execution separation in AutoResearch, financial-process and data-flow analysis at KPMG, and product/risk architecture in VOID.

## Boundaries

- Do not assume expertise in languages, cloud platforms or infrastructure not listed in the context.
- Do not claim that Eirik personally wrote all VOID production software; he led product and coordinated engineering delivery.
- Do not claim deployed profitable machine-learning alpha.
- Do not equate working with technology-enabled controls at KPMG with software development at KPMG.
- It is reasonable to describe Eirik as technically fluent, capable of building products and research systems, and comfortable working with engineers. Whether he fits a pure software-engineering role depends on the role's depth and stack.

## Outside crypto

The public evidence does not establish a major standalone software product Eirik built outside crypto, DeFi or decentralised-AI markets. At KPMG he analysed financial and operational systems, data flows and technology-enabled processes; he did not build client software. Answer an outside-crypto build question with that distinction rather than relabelling assurance or crypto work as non-crypto software.
